...
class Record {
	...
	protected static $taxonomy_elements = array(
		'subject_genre'   => 'wppp_subject_genre',
		'subject_subject' => 'wppp_subject_subject',
		'subject_people'  => 'wppp_subject_people',
		'subject_places'  => 'wppp_subject_places',
	);
	...
	public function __construct( $post_id = null ) {
		// Set asset base URL from constant, filter, or default to legacy S3 URL
		if ( defined( 'WPPP_ASSET_BASE_URL' ) ) {
			$this->asset_base = WPPP_ASSET_BASE_URL;
		} else {
			$this->asset_base = 'https://s3.amazonaws.com/bhs.assets/';
		}

		/**
		 * Filters the asset base URL for record attachments and images.
		 *
		 * Allows customization of the storage backend URL (S3, Google Drive, etc.).
		 *
		 * @since 0.2.0
		 *
		 * @param string $asset_base The base URL for assets.
		 */
		$this->asset_base = apply_filters( 'wppp_asset_base_url', $this->asset_base );
		$this->asset_base = trailingslashit( $this->asset_base );
	...
	protected function populate( $post_id ) {
		...
		if ( ! $post || 'wppp_record' !== $post->post_type ) {
			return;
		}
		...
		foreach ( self::get_dc_elements() as $element ) {
			if ( $tax = self::get_element_taxonomy( $element ) ) {
				...
				$values = get_post_meta( $post_id, 'wppp_dc_' . $element, $get_single );
			}
